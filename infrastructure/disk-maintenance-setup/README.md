...
Node hygiene (disk + DNS) for the homelab, applied uniformly. Idempotent.

```bash
ansible-playbook -i inventory setup.yml
```

- **journald cap** (`SystemMaxUse=500M`) → all hosts (lw-main, lw-c1/c2/c3, lw-db).
...
- **DNS hygiene** (`k3s_nodes`) → comments out any divergent global `DNS=` in resolved.conf so pod
  resolv.conf stays ≤3 upstreams (kills DNSConfigForming spam). Relies on link/DHCP DNS.
