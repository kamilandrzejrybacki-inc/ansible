...
3. Repoint consumers → `homelab/v2/`:
   ...
   - **3d cutover, PER NAMESPACE, ordered** via `migrate/eso/cutover.sh <ns>`: ESO (creationPolicy Owner) will not adopt a sops-owned Secret, so the script applies the ES, removes the namespace's sops files (commit+push, ArgoCD prunes the SopsSecret → Secret GC'd), force-syncs, then **gates every key's sha256 vs the pre-cutover baseline** (`~/homelab-backups/k8s-secrets-baseline-*.json`); a mismatch reverts the commit. k8s-secrets role trimmed to vault-mcp-token + codex/claude auth (`a2df6cc`); `n8n-vault-render`, `apply-secret`, `discord-webhooks` retired. Then move `argocd-app.yaml` → `apps/eso-secrets.yaml`.
