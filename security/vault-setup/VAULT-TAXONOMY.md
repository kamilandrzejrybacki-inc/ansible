...
3. Repoint consumers → `homelab/v2/`:
   - **3a DONE** — ESO: 33 ExternalSecrets / 20 namespaces generated (`migrate/eso/generate.py`), committed inert to `argocd-apps/secrets/eso/`; Application staged.
   - **3b DONE** (`4d0b84d`) — host/edge/dev-tool ansible consumers on the refs-mode helper (`vault_refs` / `vault_store_refs`, aliases = old field names; `_vault_tree` in check.yml); caddy role; claude-config; k3s-setup; the kept k8s-secrets tasks (mcp-bridges restricted bundle, hermes Codex-auth). Dead playbooks removed.
   - **3c** — cellarette's 2 `vault:` refs → `github/cellarette#token`, `openai/shared#api_key`: after step 2 (needs the cellarette-local AppRole; the legacy token can't read v2).
   - **3d cutover (after step 2), PER NAMESPACE, ordered** via `migrate/eso/cutover.sh <ns> [--commit]`: apply SecretStore+ES → wait SecretSynced → **gate: every key's sha256 vs the pre-cutover baseline** (`~/homelab-backups/k8s-secrets-baseline-*.json`) → only then remove that namespace's sops files. Start with tauto/silverbullet to observe ESO Owner-policy vs a sops-owned Secret before n8n/cellarette/hermes. Then: move `argocd-app.yaml` → `apps/eso-secrets.yaml`, disable the superseded k8s-secrets tasks (hermes discord/mcp/ntfy/crons/gh, gnhf-bot, discord-webhooks, mcp-servers except mcp-bridges, group_vars n8n-workers/github) and retire `n8n-vault-render`.
4. **Gate** (= 3d's per-namespace gate, cumulatively) + every service smoke-tested + `agents` token proven *denied* on restricted.
