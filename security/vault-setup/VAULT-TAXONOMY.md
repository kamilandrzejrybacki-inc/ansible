...
7. Follow-on (separate, NOT done): auth standardization (rule 12) — retire legacy policies (`cellarette-tools`, `n8n-secrets-reader`, `cellarette-bridge`'s broad `homelab/*` read); retire `/opt/vault/init-keys.json` root-token automation; move unseal keys off-box. Per-consumer credential *minting* (distinct creds per github/litellm/discord/DB consumer) is a rotation activity on top of the now-correct structure.
